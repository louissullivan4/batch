...
| 2 | ◑ in progress — Money, Irish VAT, and the order reducer done and property-tested (replay-equals-projection, totals never negative, void-all-to-zero, duplicate rejected). **"The same reducer imported by both apps" now holds** — `apps/till/src/order.ts` imports `reduceOrder`/`computeTotals` from `@batch/domain`, the same module `apps/api` imports (grep-verified). Remaining S2 work: the full event set and the `decide`/`reduce` split per the sprint file. |
| 3 | ☐ not started |
| 4 | ☐ not started |
| 5 | ☐ not started |
| 6 | ☐ not started |
